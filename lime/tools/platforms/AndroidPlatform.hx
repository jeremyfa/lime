...
class AndroidPlatform extends PlatformTarget {
	...
	public override function build ():Void {
		...
		for (architecture in architectures) {
			...
			var haxeParams = [ hxml, "-D", "android", "-D", "android-19" ];
			var cppParams = [ "-Dandroid", "-Dandroid-19" ];
	...
	public override function rebuild ():Void {
		...
		if (armv5) commands.push ([ "-Dandroid", "-DPLATFORM=android-19" ]);
		if (armv7) commands.push ([ "-Dandroid", "-DHXCPP_ARMV7", "-DHXCPP_ARM7", "-DPLATFORM=android-19" ]);
		if (x86) commands.push ([ "-Dandroid", "-DHXCPP_X86", "-DPLATFORM=android-19" ]);
